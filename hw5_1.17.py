...
def depth_search(graph, start):
    ...
    ## working but I think this is a breadth search
    ...
    while len(covered) < n:
        ...
        if neighbors != []:     ### change this
            while all[tier,nodenum] != []:     ## this while loop takes you to an end node
                neighbors = all[tier,nodenum]
    ...
    while len(covered) < n:
        ...
        if neighbors != []:     ### change this
            while all[tier,nodenum] != []:     ## this while loop takes you to an end node
                neighbors = all[tier,nodenum]
                nodenum = neighbors[pick]
                ## here, I need to increase pick and when pick hits end of list, increase tier & reset pick to zero
                pick += 1
                covered.append(nodenum)
